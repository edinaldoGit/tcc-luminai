...
A avaliação da arquitetura proposta será conduzida por meio de um plano experimental destinado a analisar tanto o desempenho computacional dos fluxos de processamento quanto a qualidade semântica das respostas geradas. Serão consideradas métricas relacionadas à latência no fluxo síncrono, ao tempo de processamento no fluxo assíncrono, à precisão na recuperação de evidências, à relevância das respostas e à fidelidade ao contexto recuperado. Como limitação deste estágio da pesquisa, o escopo restringe-se à análise da arquitetura EDGAR em documentos de domínio educacional normalizados no formato PDF. Embora a proposta possa futuramente ser adaptada para outros formatos documentais, esta etapa concentra-se em materiais estruturados em páginas, como apostilas, capítulos de livros e apresentações convertidas para PDF.

Este trabalho está organizado da seguinte forma: o presente capítulo contextualiza o tema, delimita o problema de pesquisa e apresenta os objetivos gerais e específicos do estudo. No Capítulo 2, é apresentada a fundamentação teórica necessária para embasar o desenvolvimento da proposta. O Capítulo 3 aborda os trabalhos relacionados e a identificação de lacunas na literatura. No Capítulo 4, é descrita a metodologia adotada, contemplando a justificativa das tecnologias selecionadas, as fases dos fluxos assíncrono e síncrono e o plano de avaliação experimental. O Capítulo 5 apresenta os experimentos piloto realizados e seus resultados preliminares. Por fim, o Capítulo 6 reúne as conclusões do trabalho e as perspectivas para trabalhos futuros.
...
Desenvolver e avaliar experimentalmente uma arquitetura de Geração Aumentada por Recuperação Multimodal (MRAG) em granularidade fina, voltada à consulta de materiais didáticos em PDF, com o propósito de fornecer ao modelo gerador evidências textuais e visuais precisas e rastreáveis, contribuindo para a redução de alucinações semânticas no contexto educacional.

\subsection{Objetivos Específicos}
\label{subsec:objs_especificos}

\begin{itemize}
    \item[-] Projetar e implementar um pipeline de ingestão documental para converter páginas 
    PDF em imagens padronizadas, detectar componentes estruturais via análise de layout e 
    isolá-los geometricamente como unidades documentais independentes;

    \item[-] Desenvolver mecanismos híbridos de extração textual localizada e descrição visual 
    automática para representar semanticamente componentes textuais e visuais de forma 
    indexável;

    \item[-] Implementar uma estratégia de indexação multimodal baseada em \textit{embeddings} 
    semânticos e metadados geométricos, associando texto extraído, descrição visual e 
    coordenadas espaciais em um banco de dados vetorial;

    \item[-] Implementar um mecanismo de recuperação semântica em granularidade fina capaz 
    de associar consultas em linguagem natural aos componentes documentais mais relevantes, 
    preservando sua rastreabilidade ao documento original;

    \item[-] Desenvolver mecanismos de engenharia de contexto para organizar as evidências 
    recuperadas antes da inferência, reduzindo ruídos contextuais e favorecendo respostas 
    mais fiéis ao material didático;

    \item[-] Desenvolver um protótipo da aplicação cliente LuminIA Reader para submissão de 
    documentos, envio de perguntas e visualização das respostas com destaque das evidências 
    documentais recuperadas;

    \item[-] Avaliar experimentalmente a viabilidade técnica da arquitetura EDGAR por meio 
    de configurações comparativas, considerando métricas de qualidade de recuperação, 
    fidelidade das respostas e desempenho computacional.
\end{itemize}