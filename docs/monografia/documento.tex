...
\autor{Francisco Edinaldo dos Santos Silva}
\titulo{EDGAR: Desenvolvimento de uma Arquitetura MRAG de Granularidade Fina Voltada para Documentos Didáticos em PDF}
\data{2026}
\local{Quixadá}
...
\newcommand{\citeb}[1]{(\citeauthoronline{#1}, \citeyear{#1})}
\newcommand{\citebp}[2]{(\citeauthoronline{#1}, \citeyear{#1}, p.~#2)}
\newcommand{\citebdois}[2]{(\citeauthoronline{#1}, \citeyear{#1}; \citeauthoronline{#2}, \citeyear{#2})}
...
\begin{document}	
	...
	\input{2-textuais/5-resultados-preliminares}
...
%	\imprimirglossario	
	\imprimirapendices
		...
		\input{3-pos-textuais/apendices/apendice-luminia-reader}
